...
def simulate_scheduling(processes, algorithm, quantum=None, priority_type=None):
    ...
    # Ensure priority field exists if priority scheduling
    if algorithm == 'priority':
        for p in procs:
            if 'pr' not in p or p['pr'] is None:
                p['pr'] = 1
    ...
    if algorithm == "fcfs":
        ...
        if quantum is None or quantum <= 0:
            return {"error": "Quantum must be positive for Round Robin"}
...
def simulate():
    ...
    if priority_type is not None:
        try:
            priority_type = int(priority_type)
        except:
            priority_type = None

    result = simulate_scheduling(processes, algorithm, quantum, priority_type)
